...
#codenames=(precise vivid wily xenial trusty wheezy jessie stretch sid)
codenames=(precise vivid wily xenial trusty yakkety wheezy jessie stretch sid)
...
codenames_ubuntu=(precise vivid wily xenial trusty yakkety)
#codenames_debian=(wheezy)
...
download_source()
{
  ...
  # Downloading file
  if wget -O $scriptpath/${source_file} -U ossec https://github.com/wazuh/wazuh/archive/master.zip; then
  ...
  do
    ...
    cp -r $scriptpath/${tmp_directory} $scriptpath/${package}_${ossec_version}
    cd $scriptpath
    cp -rp ${tmp_directory} ${package}-${ossec_version}
    tar czvf ${package}_${ossec_version}.orig.tar.gz ${package}-${ossec_version}/*
    cp -p ${package}_${ossec_version}.orig.tar.gz  $scriptpath/$package/
    rm ${package}_${ossec_version}.orig.tar.gz
    rm -rf $scriptpath/${package}_${ossec_version}
    rm -rf ${package}-${ossec_version}
  ...
  do
    ...
    do
      ...
      if [ ! -f ${changelog_file} ] ; then
        echo "Error: Couldn't find changelog file for ${source_path}/${package}-${ossec_version}" | write_log
        exit 1
      fi
...
do
  ...
  do
    ...
    s3cmd -P sync /########/apt/ s3://##############/apt/ --delete-removed --follow-symlinks
done
}
...
do
  ...
  do
    ...
    s3cmd -P sync /################/apt-dev/ s3://################/apt-dev/ --delete-removed --follow-symlinks
done
}
