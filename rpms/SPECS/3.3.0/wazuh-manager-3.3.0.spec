...
%if 0%{?el} >= 6 || 0%{?rhel} >= 6
    make -j%{_threads} DATABASE=%{_databaseoutput} TARGET=server PREFIX=%{_localstatedir}/ossec
%else
    make -j%{_threads} DATABASE=%{_databaseoutput} TARGET=server DISABLE_SYSC=yes PREFIX=%{_localstatedir}/ossec
%endif
