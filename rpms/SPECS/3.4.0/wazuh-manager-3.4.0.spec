...
%if 0%{?el} >= 6 || 0%{?rhel} >= 6
    make -j%{_threads} %{_databaseoutput} TARGET=server USE_SELINUX=yes PREFIX=%{_localstatedir}/ossec
%else
    make -j%{_threads} %{_databaseoutput} TARGET=server DISABLE_SYSC=yes USE_AUDIT=no USE_SELINUX=yes PREFIX=%{_localstatedir}/ossec
%endif
