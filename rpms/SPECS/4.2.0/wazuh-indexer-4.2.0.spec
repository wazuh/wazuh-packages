...
ODFE_FILE=${ODFE_DIR}.tar.gz
SYSTEMD_MODULE_FILE=systemd_elasticsearch_module_7.10.0.tgz

# Extract elasticsearch-oss tar.gz file
mkdir -p files/plugins
mkdir -p files/config_files
#curl -o files/$ODFE_FILE http://192.168.0.6:8000/$ODFE_FILE
#curl -o files/$SYSTEMD_MODULE_FILE http://192.168.0.6:8000/$SYSTEMD_MODULE_FILE
curl -o files/$ODFE_FILE https://packages-dev.wazuh.com/deps/wazuh-indexer/upgrade_test/$ODFE_FILE
curl -o files/$SYSTEMD_MODULE_FILE https://packages-dev.wazuh.com/deps/wazuh-indexer/upgrade_test/$SYSTEMD_MODULE_FILE
...
sed -i 's/webservice-listener-port = 9600/webservice-listener-port = 9601/' $ODFE_DIR/plugins/opendistro_performance_analyzer/pa_config/performance-analyzer.properties
sed -i 's/webservice-listener-port = 9600/webservice-listener-port = 9601/' $ODFE_DIR/plugins/opendistro_performance_analyzer/performance-analyzer-rca/pa_config/performance-analyzer.properties

sed -i 's/rpc-port = 9650/rpc-port = 9651/' $ODFE_DIR/performance-analyzer-rca/pa_config/performance-analyzer.properties
sed -i 's/rpc-port = 9650/rpc-port = 9651/' $ODFE_DIR/plugins/opendistro_performance_analyzer/pa_config/performance-analyzer.properties
sed -i 's/rpc-port = 9650/rpc-port = 9651/' $ODFE_DIR/plugins/opendistro_performance_analyzer/performance-analyzer-rca/pa_config/performance-analyzer.properties
...
sed -i 's/metrics-location = \/dev\/shm\/performanceanalyzer/metrics-location = \/dev\/shm\/wazuh-indexer-performanceanalyzer/' $ODFE_DIR/plugins/opendistro_performance_analyzer/pa_config/performance-analyzer.properties
sed -i 's/metrics-location = \/dev\/shm\/performanceanalyzer/metrics-location = \/dev\/shm\/wazuh-indexer-performanceanalyzer/' $ODFE_DIR/plugins/opendistro_performance_analyzer/performance-analyzer-rca/pa_config/performance-analyzer.properties
...
sed -i 's/metrics-db-file-prefix-path = \/tmp\/metricsdb_/metrics-db-file-prefix-path = \/tmp\/wazuh-indexer_metricsdb_/' $ODFE_DIR/plugins/opendistro_performance_analyzer/pa_config/performance-analyzer.properties
sed -i 's/metrics-db-file-prefix-path = \/tmp\/metricsdb_/metrics-db-file-prefix-path = \/tmp\/wazuh-indexer_metricsdb_/' $ODFE_DIR/plugins/opendistro_performance_analyzer/performance-analyzer-rca/pa_config/performance-analyzer.properties
...
sed -i 's!/usr/share/elasticsearch!%{INSTALL_DIR}!g' %{buildroot}%{_localstatedir}/plugins/opendistro_performance_analyzer/pa_config/supervisord.conf
sed -i 's!/usr/share/elasticsearch!%{INSTALL_DIR}!g' %{buildroot}%{_localstatedir}/plugins/opendistro_performance_analyzer/performance-analyzer-rca/pa_config/supervisord.conf
...
sed -i s'/JAVA_HOME=$2/export JAVA_HOME=$2/' %{buildroot}%{_localstatedir}/plugins/opendistro_performance_analyzer/pa_bin/performance-analyzer-agent
...
chmod +x %{buildroot}%{_localstatedir}/bin/systemd-entrypoint
...
### The following scripts are based on elasticsearch-oss and opendistro_performance_analyzer scripts and adapted to wazuh-indexer
...
## Post install script from opendistro_performance_analyzer plugin
## adapted from plugins/opendistro_performance_analyzer/install/rpm/postinst 
...
#sh %{INSTALL_DIR}/plugins/opendistro_performance_analyzer/install/rpm/postinst 1
...
if ! grep -q '## OpenDistro Performance Analyzer' %{CONFIG_DIR}/jvm.options; then
   ...
   echo "-Djava.security.policy=file://%{INSTALL_DIR}/plugins/opendistro_performance_analyzer/pa_config/es_security.policy" >> %{CONFIG_DIR}/jvm.options
fi
...
if [ "x$IS_UPGRADE" != "xtrue" ]; then
    if command -v systemctl > /dev/null; then
        ...
        echo "sh %{INSTALL_DIR}/plugins/opendistro_performance_analyzer/pa_bin/performance-analyzer-agent %{INSTALL_DIR} %{INSTALL_DIR}/jdk -d"
    fi
fi
