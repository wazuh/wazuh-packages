...
mkdir -p ${RPM_BUILD_ROOT}%{_localstatedir}/logs/{alerts,archives,cluster,firewall,ossec}
mkdir -p ${RPM_BUILD_ROOT}%{_localstatedir}/queue/{agent-groups,agent-info,agentless,agents,alerts,cluster,db,diff,fts,ossec,rids,rootcheck,syscheck,vulnerabilities}
...
mkdir -p ${RPM_BUILD_ROOT}%{_localstatedir}/var/download
...
%attr(750,ossec,ossec) %dir %{_localstatedir}/logs/cluster
...
%attr(760,root,ossec) %dir %{_localstatedir}/queue/vulnerabilities
...
%attr(770,root,ossec) %dir %{_localstatedir}/var/download
