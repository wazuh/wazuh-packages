...
if [ $1 = 2 ]; then
  if command -v service > /dev/null 2>&1 && service wazuh-manager status > /dev/null 2>&1 | grep "is running" > /dev/null 2>&1; then
    service wazuh-manager stop > /dev/null 2>&1
  elif command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 && systemctl is-active --quiet wazuh-manager > /dev/null 2>&1; then
