...
%else
    ...
    %endif
    make deps RESOURCES_URL=http://packages.wazuh.com/deps/4.1
...
if [ $1 = 2 ]; then
  if command -v service > /dev/null 2>&1 && service wazuh-agent status > /dev/null 2>&1 | grep "is running" > /dev/null 2>&1; then
    service wazuh-agent stop > /dev/null 2>&1
  elif command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 && systemctl is-active --quiet wazuh-agent > /dev/null 2>&1; then
