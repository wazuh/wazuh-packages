...
%if 0%{?el} >= 6 || 0%{?rhel} >= 6
    make -j%{_threads} TARGET=server PREFIX=%{_localstatedir}
%else
    make -j%{_threads} TARGET=server DISABLE_SYSC=yes PREFIX=%{_localstatedir}
%endif
...
if [ -d %{_localstatedir}/framework ]; then
    if [ -f %{_localstatedir}/framework/wazuh/cluster/cluster.json ]; then
        cp %{_localstatedir}/framework/wazuh/cluster/cluster.json %{_localstatedir}/tmp/cluster.json
...
if [ -f %{_localstatedir}/tmp/cluster.json ]; then
    mv %{_localstatedir}/tmp/cluster.json %{_localstatedir}/framework/wazuh/cluster/cluster.json.old
