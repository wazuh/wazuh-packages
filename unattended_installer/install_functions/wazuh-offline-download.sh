...
function offline_download() {
  ...
  if [ "$dashboard_revision" -gt 1 ] && [ "$(curl -s -o /dev/null -w "%{http_code}" "${dashboard_base_url}/${dashboard_package}" --max-time 300 --retry 5 --retry-delay 5 --retry-connrefused)" -ne "200" ]; then
    dashboard_revision=$((dashboard_revision-1))
    if [ "${package_type}" == "rpm" ]; then
      dashboard_rpm_package="wazuh-dashboard-${wazuh_version}-${dashboard_revision}.x86_64.rpm"
    else
...
# --------------------------------------------------
  ...
  do
    ...
    eval "curl -sO ${file} --max-time 300 --retry 5 --retry-delay 5 --retry-connrefused"
    if [  "${PIPESTATUS[0]}" != 0  ]; then
