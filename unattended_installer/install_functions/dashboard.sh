...
function dashboard_initialize() {
    ...
    if [ ${j} -eq 12 ]; then
        ...
        if [ -z "${force}" ]; then
            ...
            logger "When Wazuh dashboard is able to connect to your Wazuh indexer cluster, you can access the web interface https://${nodes_dashboard_ip}. The credentials are admin:${u_pass}"
        fi
    else
...
function dashboard_uninstall() {
    ...
    logger "Starting Wazuh dashboard uninstall."

    if [[ -n "${dashboardinstalled}" ]]; then
        logger -w "Removing Wazuh dashboard packages."
    ...
    if [[ -n "${dashboard_remaining_files}" ]]; then
        logger -w "Removing Wazuh dashboard files."

        elements_to_remove=(    "/etc/systemd/system/multi-user.target.wants/wazuh-dashboard.service"
