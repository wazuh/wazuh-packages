...
function installCommon_installPrerequisites() {
    ...
    if [ "${sys_type}" == "yum" ]; then
        if [ "${distro}" == "Red Hat Enterprise Linux" ] && [ "${rev_mayor}" == "8" ]; then
            dependencies=( curl wget libcap tar gnupg openssl netcat )
        else
            dependencies=( curl wget libcap tar gnupg openssl nmap-ncat )
        fi
        not_installed=()
        ...
        dependencies=( apt-transport-https curl wget libcap2-bin tar software-properties-common gnupg openssl netcat )
        not_installed=()
