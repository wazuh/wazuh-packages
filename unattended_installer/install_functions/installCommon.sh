...
function installCommon_changePasswords() {
    ...
    if [ -f "${tar_file}" ]; then
        eval "tar -xf ${tar_file} -C ${base_path} ./passwords.wazuh ${debug}"
        p_file="${base_path}/passwords.wazuh"
...
function installCommon_readPasswordFileUsers() {
    ...
    filecorrect=$(grep -Ev '^#|^\s*$' "${p_file}" | grep -Pzc '\A(\s*username:\s*\w+\s*password:\s*[A-Za-z0-9_\-]+\s*)+\Z')
