...
function indexer_initialize() {
    ...
    i=0
    until curl -XGET https://${indexer_node_ips[pos]}:${architecture_indexer_api_port}/ -uadmin:admin -k --max-time 120 --silent --output /dev/null || [ "${i}" -eq 12 ]; do
        sleep 10
        i=$((i+1))
    done
    if [ ${i} -eq 12 ]; then
...
function indexer_startCluster() {
    ...
    eval "curl --silent ${filebeat_wazuh_template} | curl -X PUT 'https://${indexer_node_ips[pos]}:${architecture_indexer_api_port}/_template/wazuh' -H 'Content-Type: application/json' -d @- -uadmin:admin -k --silent ${debug}"
    if [  "$?" != 0  ]; then
