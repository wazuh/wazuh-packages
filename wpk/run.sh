...
help() {
    ...
    echo
    echo "    -b,   --branch <branch>      [Required] Select Git branch or tag. By default: ${REFERENCE}"
    ...
    echo "    -r,   --revision <rev>       [Optional] Revision of the package. By default: ${REVISION}."
    echo "    -p,   --path <path>          [Optional] Installation path for the package. By default: ${INSTALLATION_PATH}"
    echo "    -j,   --jobs <number>        [Optional] Number of parallel jobs when compiling. By default: ${JOBS}"
    echo "    -c,   --checksum             [Optional] Whether Generate checksum or not. By default: ${CHECKSUM}"
