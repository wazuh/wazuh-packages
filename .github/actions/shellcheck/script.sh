...
if [ "${INPUT_REPORTER}" = 'github-pr-review' ]; then
  ...
  OUTPUT=$(shellcheck -f json ${INPUT_SHELLCHECK_FLAGS:-'--external-sources'} ${FILES} | jq -r ".[] | '\(.file):\(.line):\(.column):\(.level):\(.message) [SC\(.code)](https://github.com/koalaman/shellcheck/wiki/SC\(.code))'")
